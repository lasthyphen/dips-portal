...
export class UrlService {
  ...
  public mdViewerRoute = 'dips/md-viewer?mdUrl=';
  ...
  getBaseUrl(): string {
    ...
    const isDipAddress = href.match(
      /\/dips\/details\/(?<dipName>DIP\d+)[\w\-\.#]*$/i
    );
    if (isDipAddress) {
      const dipName = isDipAddress.groups.dipName;

      return (
        'https://github.com/lasthyphen/dips/tree/master/' + dipName.toUpperCase()
      );
    }
    return '';
  }
