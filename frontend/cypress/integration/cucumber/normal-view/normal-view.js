...
Then("The DIPs list should be sorted by #", () => {
  ...
  cy.get(
    "[data-cy=table-list-dips] > tbody > tr[data-cy=search-result] > td:first-child"
  ).each(($td, idx) => {
...
When("The user clicks the title column header", () => {
  cy.intercept("**/dips/findall*").as("sortRequest");
  cy.scrollTo("bottom");
  cy.get(
    "[data-cy=table-list-dips] > thead > tr > th.mat-column-title .headerContent"
  ).click();
  cy.wait("@sortRequest");
...
When("The user clicks the status column header", () => {
  cy.scrollTo("bottom");
  cy.get(
    "[data-cy=table-list-dips] > thead > tr > th.mat-column-status .headerContent"
  ).click();
  cy.wait("@DIPs");
});
...
Then("The DIPs list should have length {string}", (length) => {
